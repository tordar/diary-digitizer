...
Rules:
- entry_type: "text" = prose writing; "image" = drawing/photo with no readable text; "mixed" = text + drawings; "special" = structured page (table, habit tracker, list, index, song lyrics, poems, quotes). Song lyrics or poems copied into the diary are common — classify them as "special" and transcribe the text exactly as written.
...
- transcription: transcribe every line of text on the page, working top to bottom. Be thorough — do not stop early or summarise. For difficult or unclear words, first complete the full sentence or paragraph, then revisit the unclear word using the full surrounding context (sentence grammar, topic, what makes sense in Norwegian, handwriting patterns elsewhere on the page). A word that looks ambiguous in isolation is often obvious once you have the whole sentence. Only use [?] if a word remains unreadable even after considering full context. Never skip a section because it is dense or hard to read. Preserve original language, spelling, and line breaks. null for pure image entries.
